...
只有當 $x^2-4y \\geq 0$ 才有實數解，再檢查 $x^2-4y$ 為完全平方數 且 $\\frac{x \\pm \\sqrt{x^2-4y}}{2}$ 必須是整數，只要有一個條件不滿足則答案為 $0$ 。
...
觀察到當前 $k$ 項為滿足 $cnt_1 > cnt_0$ 時， 前 $k + 1$ 項也會滿足；相反地，前 $k$ 項不滿足 $cnt_1 > cnt_0$ 時，前 $k - 1$ 項也會不滿足。  
