...
### Solution
先找到 $c \\in \\mathbb{N}$ 滿足 $2^c \\leq X < 2^{c+1}$ ，一個 $0, 1, 2, \\ldots, c-1$ 的數列會貢獻 $2^c$ 個遞增子序列，在這個數列最左端嘗試插入一個數 $t\\in [0, c-1]$ ，它會貢獻 $2^{c-t-1}$ 個遞增子序列，接著注意到 $$0\\leq X-2^c < 2^c$$ ，我們只需要保證插入的數是遞減數列使，$X-2^c$ 就可以由若干個 $2^{c-t-1}$ 組合出來，這樣構造數列長度最長為 $2\\lg X$ ，當 $X=10^{18}$ 大約為 $120$ 。
