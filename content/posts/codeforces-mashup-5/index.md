...
兩個平台重疊的部份不好處理，先解決不重疊的情況。用雙指標找到最大的 $r$ 滿足 $x_r - x_l \\leq k$ ， 把每個 $[l,r]$ 再加上 $[l,r]$ 包含點的個數包成 tuple 丟進 priority queue。接著同樣用雙指標由左往右枚舉每一個點當作第一個平台的開頭，在距離不超過 $k$ 的情況下不斷增加右界，再從 priority queue 中取出包含最多點的 tuple
。若取出 tuple 的 $[l, r]$ 和第一個平台重疊，就 pop 掉繼續取，直到不重疊然後更新答案。

關於重疊的部份，不難發現兩個平台頭尾相接一定比重疊好，因此可以想像成一個長度 $2k$ 的平台再跑一次雙指標就好。
