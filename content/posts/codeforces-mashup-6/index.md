...
### Solution
正面算不好算，可以嘗試分別扣掉滿足 $x+y$ 和 $y-x$ 的 pair 再把重複的加回來，難點會是求重複的個數。把集合中所有 $x+y$ 和 $y-x$ 畫在二維平面上，不難發現重複的個數就是所有交點是整數點的個數，且只有當集合中兩點 $t_1,\\ t_2$ 的差值是偶數的時候交點才會是整數點，因此可以把集合分成奇偶兩個部份再用梯形公式把他們兩個加起來。
...
### Solution
先找到 $c \\in \\mathbb{N}$ 滿足 $2^c \\leq X < 2^{c+1}$ ，一個 $0, 1, 2, \\ldots, c-1$ 的數列會貢獻 $2^c$ 個遞增子序列，在這個數列最左端嘗試插入一個數 $t\\in [0, c-1]$ ，它會貢獻 $2^{c-t-1}$ 個遞增子序列，接著注意到 $$0\\leq X-2^c < 2^c$$ ，我們只需要保證左端插入的數是遞減數列，$X-2^c$ 就可以由若干個 $2^{c-t-1}$ 組合出來，這樣構造數列長度最長為 $2\\lg X$ ，當 $X=10^{18}$ 大約為 $120$ 。
...
可以隨便挑一個點 DFS ，所有 leaf 深度的 parity 都要一樣最小值才會是 $1$ 。
