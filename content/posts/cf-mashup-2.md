...
再考慮最初的數列 $a$ ，利用剛剛得出的結論，一個長度為 $n$ 的數列最多也只會貢獻 $n$ 的分數，因此最多連續執行操作一 $\\min(2n + 1, d)$ 天。假設第 $i$ 第一次執行操作二 ，答案為 $\\max(cnt_i + \\left\\lfloor \\frac{n - i}{2} \\right\\rfloor)$，其中 $cnt_i$ 為 $a_j = j$ 的個數，  
$1\\leq i \\leq \\min(2n+1, d)$ 。
