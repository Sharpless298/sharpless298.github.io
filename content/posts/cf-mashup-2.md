...
- 若產品價格不超過 $a_i$，顧客會購買一棵樹並留下好評；
- 否則，若產品價格不超過 $b_i$，顧客會購買一棵樹但留下負評；
- 否則，顧客將不會購買任何樹。
...
離散化後第二和第三個觀察可以用差分 + 前綴和解決，之後暴力枚舉 $S$ 裡面的元素。

離散化 $O(n \\log n)$ ，差分 + 前綴和 $O(n)$ ，暴力枚舉 $S$ 裡面每一個元素 $O(n)$ ，總時間複雜度 $O(n \\log n)$ 。
